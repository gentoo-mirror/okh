...
IUSE="doc systemd test webengine webkit ${QTC_PLUGINS[@]%:*} ${EXTRA_USE}"
...
src_prepare() {
	...
	if use cmake2; then
		CMPM2=src/plugins/cmakeprojectmanager2
		git clone https://github.com/h4tr3d/cmakeprojectmanager2.git ${CMPM2} || die
		mv ${CMPM2}/cmakeprojectmanager.pro ${CMPM2}/cmakeprojectmanager2.pro || die
		mv ${CMPM2}/cmakeprojectmanager_dependencies.pri ${CMPM2}/cmakeprojectmanager2_dependencies.pri || die
		sed -i -e '/^include/d;/^QTCREATOR_/d;/^IDE/d;/^INCLUDEPATH/d;/^#/d' ${CMPM2}/cmakeprojectmanager2.pro || die
		echo 'include(../../qtcreatorplugin.pri)' >> ${CMPM2}/cmakeprojectmanager2.pro
		sed -i -e '/^SUBDIRS/a \ \ \ \ cmakeprojectmanager2 \\' src/plugins/plugins.pro || die
	fi
